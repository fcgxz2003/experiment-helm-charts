...
output_file="../fig11_output/50pod_5G_random_epoch3.txt"

tmp_dir=$(mktemp -d)

pod="dragonfly-peer"
minio_address=$MINIO_ADDRESS
model="http://$minio_address/models/5G.bin?x=1002"
container="peer"
namespace="dragonfly-system"

mkdir -p ../fig11_output
touch ../fig11_output/50pod_5G_random_epoch3.txt
...
command_delete() {
  kubectl exec -it $pod-$1 -c peer -n $namespace -- rm -rf 5G.bin?x=1002
}
