...
# 先预热10个，再下载50个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1306"
...
for i in $(seq 0 59);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1306 &
done
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1307"
...
for i in $(seq 0 79);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1307 &
done
