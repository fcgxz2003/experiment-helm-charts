...
# ------------------------------------ 没有预热
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1208"
...
for i in $(seq 0 99);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1208 &
done
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1209"
...
for i in $(seq 0 109);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1209 &
done
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1210"
...
for i in $(seq 0 129);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1210 &
done
...
# 预热够50个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1211"
...
for i in $(seq 0 149);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1211 &
done
