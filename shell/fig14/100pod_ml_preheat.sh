...
# ------------------------------------ 没有预热
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1008"
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1009"
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1010"
...
# 预热够50个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1011"
