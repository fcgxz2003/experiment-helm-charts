...
# ------------------------------------ 没有预热
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1308"
...
for i in $(seq 0 99);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 10G.bin?x=1308 &
done
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1309"
...
for i in $(seq 0 109);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 10G.bin?x=1309 &
done
...
# 预热够30个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1310"
...
for i in $(seq 0 129);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 10G.bin?x=1310 &
done
...
# 预热够50个
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/10G.bin?x=1311"
...
for i in $(seq 0 149);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 10G.bin?x=1311 &
done
