...
# ------------------------------------ 没有预热
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1203"
...
for i in $(seq 0 29);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1203 &
done
...
# 重新预热一个模型，预热10个Pod
tmp_dir=$(mktemp -d)
model="http://$minio_address/models/1G.bin?x=1204"
...
for i in $(seq 0 39);do
    kubectl exec -it $pod-$i -c peer -n $namespace -- rm -rf 1G.bin?x=1204 &
done
